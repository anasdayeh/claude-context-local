...
def test_resume_skips_completed_files(tmp_path):
    ...
    class ResumeAwareChunker(MultiLanguageChunker):
        def chunk_file(self, file_path: str):
            if file_path.endswith("a.py"):
                raise AssertionError("resume should skip a.py")
            return super().chunk_file(file_path)

    index_manager = CodeIndexManager(str(index_dir))
    chunker = ResumeAwareChunker(root_path=str(project_dir))
